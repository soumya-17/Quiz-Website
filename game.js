...
setStyles = (styles) => {
    ...
    if (a.length <= 20)
        document.body.style.backgroundColor = a; //Assuming entered a Color name or Hex Value
    else if (a[0] == 'l')
        document.body.style.backgroundImage = a; //Assuming it is a linear Gradient
    else
        document.body.style.backgroundImage = "url(" + a + ")"; //Assuming it is the link of an image
    ...
    var elements2 = ['#progressText', '.hud-prefix_ch', '.hud-prefix_cd', '#question'];
    elements2.forEach(x => {
...
getNewQuestion = (availableQuestions) => {
    ...
    if (currentQuestion.question.length > 330) { //If question is too lengthy
    ...
    choices.forEach(choice => {
        choice.addEventListener('click', e => {
            ...
            let isCorrect = selectedAnswer == currentQuestion.answer ? 'correct' : 'incorrect'
...
var count = 241; //The amount of time for the particlar quiz
var interval = setInterval(function() {
        var minutes = Math.floor(count / 60);
        var seconds = count - minutes * 60;
        if (document.querySelector(".hud-prefix_cd"))
            document.querySelector(".hud-prefix_cd").innerText = minutes + " Minutes " + seconds + " Seconds ";
        count--;
        if (count <= -1) {
            clearInterval(interval);
            document.querySelector(".hud-prefix_cd").style.color = "red";
            document.querySelector(".hud-prefix_ch").style.visibility = "hidden";
            document.querySelector(".hud-prefix_cd").innerHTML = "Sorry, You're Out of Time";
